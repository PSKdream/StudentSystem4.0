...
###################################ปุ่มหน้าแรก################################################
	...
	def insert_data (c1,c2,c3,c4,c5,c6,c7,c8,c9):
		...
		if(c10==3) :
			areaa = "ต่างจังหวัด"'''
		with conn:
			c.execute(""" INSERT INTO data VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """,(None,dt,c1,c2,c3,c4,c5,c6,c7,c8,c9))
			conn.commit()
		#print("insert")
	def add_data():
		global trueID
		if (Varshowroomtest.get()==""or Varshowtest.get()=="" or VarshowID.get()=="" 
			or VarshowNameTH.get()=="" or SHschool.get()=='' 
			 or VarshowDate.get()==''or VarshowGender.get()==''   ):#CheckVar.get() == 0):
				...
				if(search == ()):
					...
					if(search==()):
						...
						insert_data(int(trueID),Varshowroomtest.get(),Varshowroom.get(),VarshowID.get(),VarshowNameTH.get()
							,VarshowDate.get(),VarshowGender.get(),SHschool.get(),order)
						with conn:
